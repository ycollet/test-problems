...
if nargin<6 then
  error('initialize: Usage: initialize(dimension,nGaussian,upper,lower,globalvalue,ratio)');
end

if dimension<=1 | nGaussian<=0 | upper<=lower | globalvalue<=0 | ratio<=0 | ratio>=1
  error('initialize: Incorrect parameter values!');
end
...
for i=1:nGaussian
  for j=1:dimension-1        // totally n(n-1)/2 rotation matrice
      for k=j+1:dimension
        r      = e;
        alpha  = rand(1,1)*%pi/2-%pi/4;// random rotation angle [-pi/4,pi/4]
        r(j,j) = cos(alpha);
        r(j,k) = sin(alpha);
        r(k,j) = -sin(alpha);
        r(k,k) = cos(alpha);
...
_variance = rand(nGaussian,dimension)*variancerange+0.05;  // add 0.05 to avoid zero variance

for i=1:nGaussian
  covmatrix        = diag(_variance(i,:));
