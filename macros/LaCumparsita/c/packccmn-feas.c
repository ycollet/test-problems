/* packccmn-feas.f -- translated by f2c (version 20000817).
...
/*     and R, the goal is to solve the feasibility problem: */

/*     d( pi, pj )^2 >= ( ri + rj )^2, for all i < j, */

/*     d( 0, pi )^2 <= ( R - ri )^2, for all i. */

/*     where d(.,.) is the Euclidian distance. */
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_inip__(n, x, l, u, m, lambda, rho, equatn, 
	linear, nd_in__, nite_in__, iterad_in__, objrad_in__, seed_in__)
integer *n;
doublereal *x, *l, *u;
integer *m;
...
doublereal *iterad_in__, *objrad_in__, *seed_in__;
{
...
/*     Radii of circular items and object, respectively */
    packdata_1.iterad = *iterad_in__;
    packdata_1.objrad = *objrad_in__;
...
/*     Number of constraints (equalities plus inequalities) */
    *m = packdata_1.nite + packdata_1.nite * (packdata_1.nite - 1) / 2;
...
} /* packccmn_feas_inip__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_evalf__(n, x, f, flag__)
integer *n;
doublereal *x, *f;
integer *flag__;
{
...
/*              equal to zero. */
    ...
    *flag__ = 0;
    *f = 0.;
} /* packccmn_feas_evalf__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_evalg__(n, x, g, flag__)
integer *n;
doublereal *x, *g;
integer *flag__;
{
...
} /* packccmn_feas_evalg__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_evalh__(n, x, hlin, hcol, hval, hnnz, 
	flag__)
integer *n;
doublereal *x;
integer *hlin, *hcol;
...
/*              everything was o.k. you must set it equal to zero. */
    ...
    *flag__ = 0;
    *hnnz = 0;
} /* packccmn_feas_evalh__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_evalc__(n, x, ind, c__, flag__)
integer *n;
doublereal *x;
integer *ind;
doublereal *c__;
...
/* Computing 2nd power */
    ...
    } else if (packdata_1.nite + 1 <= *ind && *ind <= packdata_1.nite + 
	    packdata_1.nite * (packdata_1.nite - 1) / 2) {
	i1 = packdata_1.pair[(*ind - packdata_1.nite << 1) - 2];
	i2 = packdata_1.pair[(*ind - packdata_1.nite << 1) - 1];
/* Computing 2nd power */
...
} /* packccmn_feas_evalc__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_evaljac__(n, x, ind, jcvar, jcval, jcnnz, 
	flag__)
integer *n;
doublereal *x;
integer *ind, *jcvar;
...
/*     COMMON BLOCKS */
    ...
    *flag__ = 0;
    if (1 <= *ind && *ind <= packdata_1.nite) {
	*jcnnz = packdata_1.ndim;
	i__1 = packdata_1.ndim;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    jcvar[i__] = packdata_1.ndim * (*ind - 1) + i__;
	    jcval[i__] = x[packdata_1.ndim * (*ind - 1) + i__] * 2.;
	}
    } else if (packdata_1.nite + 1 <= *ind && *ind <= packdata_1.nite + 
	    packdata_1.nite * (packdata_1.nite - 1) / 2) {
	i1 = packdata_1.pair[(*ind - packdata_1.nite << 1) - 2];
...
} /* packccmn_feas_evaljac__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_evalhc__(n, x, ind, hclin, hccol, hcval, 
	hcnnz, flag__)
integer *n;
doublereal *x;
...
/*     COMMON BLOCKS */
	...
	*hcnnz = packdata_1.ndim;
	i__1 = packdata_1.ndim;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    hclin[i__] = packdata_1.ndim * (*ind - 1) + i__;
	    hccol[i__] = packdata_1.ndim * (*ind - 1) + i__;
	    hcval[i__] = 2.;
	}
    } else if (packdata_1.nite + 1 <= *ind && *ind <= packdata_1.nite + 
	    packdata_1.nite * (packdata_1.nite - 1) / 2) {
...
} /* packccmn_feas_evalhc__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_evalhlp__(n, x, m, lambda, p, hp, goth, 
	flag__)
integer *n;
doublereal *x;
integer *m;
...
/*              must set it equal to zero. */
    ...
    /* Function Body */
    *flag__ = -1;
} /* packccmn_feas_evalhlp__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int packccmn_feas_endp__(n, x, l, u, m, lambda, rho, equatn, 
	linear)
integer *n;
doublereal *x, *l, *u;
integer *m;
