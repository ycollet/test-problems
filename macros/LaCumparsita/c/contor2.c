/* contor2.f -- translated by f2c (version 20000817).
...
/*     y'' = a y^2 + b y + c which is closest to a set of given data */
/*     points, considering y, a, b, and c as unknowns. The data (ti, yi) */
/*     are such that yi is a random [-1, 1] perturbation of a true */
/*     discrete solution. The true discrete solution is such that */
/*     y(0) = y'(0) = 1. */
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_inip__(n, x, l, u, m, lambda, rho, equatn, 
	linear, n_in__)
integer *n;
doublereal *x, *l, *u;
integer *m;
...
/*     Number of points, including extremes */
    ...
    /* Function Body */
    probdata_1.npun = *n_in__;
    probdata_1.h__ = 1. / (doublereal) (probdata_1.npun - 1);
...
/*     Initial point */
    ...
    x[probdata_1.npun + 1] = drand_(&seed);
    x[probdata_1.npun + 2] = drand_(&seed);
    x[probdata_1.npun + 3] = drand_(&seed);
...
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_evalf__(n, x, f, flag__)
integer *n;
doublereal *x, *f;
integer *flag__;
{
...
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_evalg__(n, x, g, flag__)
integer *n;
doublereal *x, *g;
integer *flag__;
{
...
/*     COMMON BLOCKS */
    ...
    *flag__ = 0;
    ...
    i__1 = *n;
    for (i__ = probdata_1.npun + 1; i__ <= i__1; ++i__) {
	g[i__] = 0.;
    }
} /* contor2_evalg__ */
...
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_evalh__(n, x, hlin, hcol, hval, nnzh, flag__)
integer *n;
doublereal *x;
integer *hlin, *hcol;
doublereal *hval;
...
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_evalc__(n, x, ind, c__, flag__)
integer *n;
doublereal *x;
integer *ind;
doublereal *c__;
...
/* Computing 2nd power */
    d__2 = x[*ind + 1];
    *c__ = (x[*ind + 2] - x[*ind + 1] * 2. + x[*ind]) / (d__1 * d__1) - (x[
	    probdata_1.npun + 1] * (d__2 * d__2) + x[probdata_1.npun + 2] * x[
	    *ind + 1] + x[probdata_1.npun + 3]);
} /* contor2_evalc__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_evaljac__(n, x, ind, indjac, valjac, nnzjac, 
	flag__)
integer *n;
doublereal *x;
integer *ind, *indjac;
...
/* Computing 2nd power */
    d__1 = probdata_1.h__;
    valjac[1] = 1. / (d__1 * d__1);
    indjac[2] = *ind + 1;
/* Computing 2nd power */
    d__1 = probdata_1.h__;
    valjac[2] = -2. / (d__1 * d__1) - (x[*ind + 1] * 2. * x[probdata_1.npun + 
	    1] + x[probdata_1.npun + 2]);
    indjac[3] = *ind;
/* Computing 2nd power */
    d__1 = probdata_1.h__;
    valjac[3] = 1. / (d__1 * d__1);
    indjac[4] = probdata_1.npun + 1;
/* Computing 2nd power */
    d__1 = x[*ind + 1];
    valjac[4] = -(d__1 * d__1);
    indjac[5] = probdata_1.npun + 2;
    valjac[5] = -x[*ind + 1];
    indjac[6] = probdata_1.npun + 3;
    valjac[6] = -1.;
} /* contor2_evaljac__ */

/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_evalhc__(n, x, ind, hclin, hccol, hcval, nnzhc, 
	flag__)
integer *n;
doublereal *x;
...
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_evalhlp__(n, x, m, lambda, p, hp, goth, flag__)
integer *n;
doublereal *x;
integer *m;
...
/*     ****************************************************************** */
/*     ****************************************************************** */
/* Subroutine */ int contor2_endp__(n, x, l, u, m, lambda, rho, equatn, 
	linear)
integer *n;
doublereal *x, *l, *u;
integer *m;
