...
c     f'(x) is the Jacobian matrix for the swirling flow between
c     disks problem at the point x.
c
c     The subroutine statement is
c
c       subroutine ssfdjs(n,x,s,y,eps,nint)
c
c     where
c
c       n is an integer variable.
c         On entry n is the number of variables. 14*nint.
...
c     **********
      ...
      real h, hm, nf, rhoijh, temp
      real dwf(fdeg+1,cpts+fdeg), dwg(gdeg+1,cpts+gdeg),
     +     rhnfhk(cpts,0:dim,0:dim,0:mdeg), rho(cpts), wf(fdeg+1),
     +     wg(gdeg+1)
