...
c
c     **********
      real zero,p5,one,two,four
      parameter (zero=0.0,p5=0.5,one=1.0,two=2.0,four=4.0)
      real mu1,mu2
      parameter (mu1=1.0,mu2=2.0)

      integer i,j,k
      real hx,hy,vl,vr,vb,vt,v,dvdx,dvdy,dvdxhx,dvdyhy,area,gradv,
     +     dpsipp,dpsip,t1,t2,dzdx,dzdxhx,dzdy,dzdyhy,z,zr,zt,zb,zl
...
c     Computation of H(x)*s over the lower triangular elements.
      ...
      do 30 j = 0,ny
         do 20 i = 0,nx
            ...
            call sodcps(gradv,mu1,mu2,t1,t2,dpsipp,'PSIPP',lambda)
