      subroutine sficfj(n,x,fvec,fjac,ldfjac,task,r,nint)
      character*(*) task
...
c
c       subroutine sficfj(n,x,fvec,fjac,ldfjac,task,r,nint)
c
c     where
c
...
c     **********
      ...
      real zero,one,two,three,six,twelve
      parameter (zero=0.0,one=1.0,two=2.0,three=3.0,six=6.0,twelve=12.0)

      integer i,j,k,m,var,eqn
      real h,xt,hm,rhoijh,nf
      real rho(cpts),dw(deg+1,cpts+deg),rhnfhk(cpts,0:dim,0:dim,0:deg),
     +     w(deg+1)
...
c        Set up the collocation equations.
  ...
  120          continue
  130       continue
            if (task.eq.'F' .or. task.eq.'FJ') fvec(eqn+k) = w(5) -
     +      r* (w(2)*w(3)-w(1)*w(4))
            if (task.eq.'J' .or. task.eq.'FJ') then
               do 140 j = 1,npi
