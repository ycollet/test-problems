      subroutine sgl2fg(nx,ny,x,f,fgrad,task,w,vornum)
      character*(*) task
      integer nx,ny,vornum
      real f
      real x(4*nx*ny),fgrad(4*nx*ny),w(4* (nx+1)* (ny+1))
c
c     Subroutine sgl2fg
c
c     This subroutine computes the function and gradient of the
c     Two-Dimensional Ginzburg-Landau Superconductivity problem.
c
c     The subroutine statement is:
c
c       subroutine sgl2fg(nx,ny,x,f,fgrad,task,w)
c
c     where
c
...
c
c       x is a real array of dimension 4*nx*ny.
...
c            or 'FG'.
c
c       fgrad is a real array of dimension 4*nx*ny.
...
c
c       w is a real work array of dimension 4*(nx+1)(ny+1).
c
...
c     Pack work array.
   ...
   20 continue
      ...
      call ssgl2(nx,ny,w(1),w(itemp+1),w(2*itemp+1),w(3*itemp+1),f,
     +fgrad(1),fgrad(nx*ny+1),fgrad(2*nx*ny+1),fgrad(3*nx*ny+1),task,
     +vornum)
...
c     Unpack work array
   ...
   40 continue
      ...
      subroutine ssgl2(nx,ny,x,y,vpotx,vpoty,f,gradx,grady,gradax,
     +                 graday,task,vornum)
      character*(*) task
      integer nx,ny,vornum
      real x(nx+1,ny+1),y(nx+1,ny+1),vpotx(nx+1,ny+1),vpoty(nx+1,ny+1),
     +     gradx(nx,ny),grady(nx,ny),gradax(nx,ny),graday(nx,ny)
c
c     Subroutine ssgl2
c
c     This subroutine computes the function and gradient of the
...
c     The subroutine statement is:
c
c       subroutine ssgl2(nx,ny,x,y,vpotx,vpoty,f,
c    +                  gradx,grady,gradax,graday,task)
c
c     where
c
c       nx is an integer variable.
...
c
c       vpotx is a real array of dimension nx*ny.
...
c
c       vpoty is a real array of dimension nx*ny.
...
c            or 'FG'.
c
c       gradx is a real array of dimension nx*ny.
...
c     ******
      ...
      parameter (zero=0.0,one=1.0,two=2.0,three=3.0,four=4.0,five=5.0)

      integer i,j
      real f,fcond,fkin,ffield,bave,tkappa,xpt,ypt,arg,x1,x2,xy,delsq,
     +     hx,hy,sqn,fkinx1,fkinx2,fkiny1,fkiny2,sfac,cfac,pi,sqrtv

c     Initialize.

      tkappa = five
      hx = sqrt(vornum/two)*three/real(nx)
      hy = sqrt(vornum/two)*three*sqrt(three)/real(ny)
      sqn = real(nx*ny)
      pi = four*atan(one)
      bave = two*pi*vornum*tkappa/ (sqn*hx*hy)
      sqrtv = sqrt(real(vornum))*pi
...
c        Compute the Magnetic Field Energy Density.
         ...
         do 120 i = 1,nx
            do 110 j = 1,ny
               ...
               xy = xy - (vpotx(i,j+1)-vpotx(i,j))/hy
               ffield = ffield + xy**2
  110       continue
  120    continue
...
c        Kinetic Energy Part, Interior Points
         ...
         do 160 i = 2,nx
            do 150 j = 2,ny
               fkinx1 = (two/ (hx*hx*sqn))*
     +         (x(i+1,j)-x(i,j)*cos(hx*vpotx(i,j))+
     +         y(i,j)*sin(hx*vpotx(i,j)))
               fkinx2 = (two/ (hx*hx*sqn))*
     +         (y(i+1,j)-y(i,j)*cos(hx*vpotx(i,j))-
     +         x(i,j)*sin(hx*vpotx(i,j)))
               fkiny1 = (two/ (hy*hy*sqn))*
     +         (x(i,j+1)-x(i,j)*cos(hy*vpoty(i,j))+
     +         y(i,j)*sin(hy*vpoty(i,j)))
               fkiny2 = (two/ (hy*hy*sqn))*
     +         (y(i,j+1)-y(i,j)*cos(hy*vpoty(i,j))-
     +         x(i,j)*sin(hy*vpoty(i,j)))
     ...
     +         sin(hy*vpoty(i,j))*fkiny1 + (-cos(hy*vpoty(i,j)))*fkiny2
               gradax(i,j) = gradax(i,j) +
     +         fkinx1* (hx*x(i,j)*sin(hx*vpotx(i,j))+
     +         hx*y(i,j)*cos(hx*vpotx(i,j))) +
     +         fkinx2* (hx*y(i,j)*sin(hx*vpotx(i,j))-
     +         hx*x(i,j)*cos(hx*vpotx(i,j))) + ffield/hy
               graday(i,j) = graday(i,j) +
     +         fkiny1* (hy*x(i,j)*sin(hy*vpoty(i,j))+
     +         hy*y(i,j)*cos(hy*vpoty(i,j))) +
     +         fkiny2* (hy*y(i,j)*sin(hy*vpoty(i,j))-
     +         hy*x(i,j)*cos(hy*vpoty(i,j))) - ffield/hx
               fkinx1 = (two/ (hx*hx*sqn))*
     +         (x(i,j)-x(i-1,j)*cos(hx*vpotx(i-1,j))+
     +         y(i-1,j)*sin(hx*vpotx(i-1,j)))
               fkinx2 = (two/ (hx*hx*sqn))*
     +         (y(i,j)-y(i-1,j)*cos(hx*vpotx(i-1,j))-
     +         x(i-1,j)*sin(hx*vpotx(i-1,j)))
               fkiny1 = (two/ (hy*hy*sqn))*
     +         (x(i,j)-x(i,j-1)*cos(hy*vpoty(i,j-1))+
     +         y(i,j-1)*sin(hy*vpoty(i,j-1)))
               fkiny2 = (two/ (hy*hy*sqn))*
     +         (y(i,j)-y(i,j-1)*cos(hy*vpoty(i,j-1))-
...
c        Bottom J = 1
     ...
     +      x(i,1)*cos(hx*vpotx(i,1))+y(i,1)*sin(hx*vpotx(i,1)))
            fkinx2 = (two/ (hx*hx*sqn))* (y(i+1,1)-
     +      y(i,1)*cos(hx*vpotx(i,1))-x(i,1)*sin(hx*vpotx(i,1)))
            fkiny1 = (two/ (hy*hy*sqn))* (x(i,2)-
     +      x(i,1)*cos(hy*vpoty(i,1))+y(i,1)*sin(hy*vpoty(i,1)))
            fkiny2 = (two/ (hy*hy*sqn))* (y(i,2)-
     +      y(i,1)*cos(hy*vpoty(i,1))-x(i,1)*sin(hy*vpoty(i,1)))
            ffield = (vpotx(i,1)-vpotx(i,2))/hy +
     +      (vpoty(i+1,1)-vpoty(i,1))/hx
            ffield = (two* (tkappa**2)/sqn)*ffield
            gradx(i,1) = gradx(i,1) + (-cos(hx*vpotx(i,1)))*fkinx1 +
     +      (-sin(hx*vpotx(i,1)))*fkinx2 +
     +      (-cos(hy*vpoty(i,1)))*fkiny1 + (-sin(hy*vpoty(i,1)))*fkiny2
            grady(i,1) = grady(i,1) + sin(hx*vpotx(i,1))*fkinx1 +
     +      (-cos(hx*vpotx(i,1)))*fkinx2 + sin(hy*vpoty(i,1))*fkiny1 +
     +      (-cos(hy*vpoty(i,1)))*fkiny2
            gradax(i,1) = gradax(i,1) + fkinx1*
     +      (hx*x(i,1)*sin(hx*vpotx(i,1))+
     +      hx*y(i,1)*cos(hx*vpotx(i,1))) +
     +      fkinx2* (hx*y(i,1)*sin(hx*vpotx(i,1))-
     +      hx*x(i,1)*cos(hx*vpotx(i,1))) + ffield/hy
            graday(i,1) = graday(i,1) + fkiny1*
     +      (hy*x(i,1)*sin(hy*vpoty(i,1))+
     +      hy*y(i,1)*cos(hy*vpoty(i,1))) +
     +      fkiny2* (hy*y(i,1)*sin(hy*vpoty(i,1))-
     +      hy*x(i,1)*cos(hy*vpoty(i,1))) - ffield/hx
            fkinx1 = (two/ (hx*hx*sqn))* (x(i,1)-
     +      x(i-1,1)*cos(hx*vpotx(i-1,1))+y(i-1,1)*sin(hx*vpotx(i-1,1)))
            fkinx2 = (two/ (hx*hx*sqn))* (y(i,1)-
     +      y(i-1,1)*cos(hx*vpotx(i-1,1))-x(i-1,1)*sin(hx*vpotx(i-1,1)))
            fkiny1 = (two/ (hy*hy*sqn))* (x(i,ny+1)-
     +      x(i,ny)*cos(hy*vpoty(i,ny))+y(i,ny)*sin(hy*vpoty(i,ny)))
            fkiny2 = (two/ (hy*hy*sqn))* (y(i,ny+1)-
     +      y(i,ny)*cos(hy*vpoty(i,ny))-x(i,ny)*sin(hy*vpoty(i,ny)))
...
c        Left I = 1.
     ...
     +      x(1,j)*cos(hx*vpotx(1,j))+y(1,j)*sin(hx*vpotx(1,j)))
            fkinx2 = (two/ (hx*hx*sqn))* (y(2,j)-
     +      y(1,j)*cos(hx*vpotx(1,j))-x(1,j)*sin(hx*vpotx(1,j)))
            fkiny1 = (two/ (hy*hy*sqn))* (x(1,j+1)-
     +      x(1,j)*cos(hy*vpoty(1,j))+y(1,j)*sin(hy*vpoty(1,j)))
            fkiny2 = (two/ (hy*hy*sqn))* (y(1,j+1)-
     +      y(1,j)*cos(hy*vpoty(1,j))-x(1,j)*sin(hy*vpoty(1,j)))
            ffield = (vpotx(1,j)-vpotx(1,j+1))/hy +
     +      (vpoty(2,j)-vpoty(1,j))/hx
            ffield = (two* (tkappa**2)/sqn)*ffield
            gradx(1,j) = gradx(1,j) + (-cos(hx*vpotx(1,j)))*fkinx1 +
     +      (-sin(hx*vpotx(1,j)))*fkinx2 +
     +      (-cos(hy*vpoty(1,j)))*fkiny1 + (-sin(hy*vpoty(1,j)))*fkiny2
            grady(1,j) = grady(1,j) + sin(hx*vpotx(1,j))*fkinx1 +
     +      (-cos(hx*vpotx(1,j)))*fkinx2 + sin(hy*vpoty(1,j))*fkiny1 +
     +      (-cos(hy*vpoty(1,j)))*fkiny2
            gradax(1,j) = gradax(1,j) + fkinx1*
     +      (hx*x(1,j)*sin(hx*vpotx(1,j))+
     +      hx*y(1,j)*cos(hx*vpotx(1,j))) +
     +      fkinx2* (hx*y(1,j)*sin(hx*vpotx(1,j))-
     +      hx*x(1,j)*cos(hx*vpotx(1,j))) + ffield/hy
            graday(1,j) = graday(1,j) + fkiny1*
     +      (hy*x(1,j)*sin(hy*vpoty(1,j))+
     +      hy*y(1,j)*cos(hy*vpoty(1,j))) +
     +      fkiny2* (hy*y(1,j)*sin(hy*vpoty(1,j))-
     +      hy*x(1,j)*cos(hy*vpoty(1,j))) - ffield/hx
            fkinx1 = (two/ (hx*hx*sqn))* (x(nx+1,j)-
     +      x(nx,j)*cos(hx*vpotx(nx,j))+y(nx,j)*sin(hx*vpotx(nx,j)))
            fkinx2 = (two/ (hx*hx*sqn))* (y(nx+1,j)-
     +      y(nx,j)*cos(hx*vpotx(nx,j))-x(nx,j)*sin(hx*vpotx(nx,j)))
            fkiny1 = (two/ (hy*hy*sqn))* (x(1,j)-
     +      x(1,j-1)*cos(hy*vpoty(1,j-1))+y(1,j-1)*sin(hy*vpoty(1,j-1)))
            fkiny2 = (two/ (hy*hy*sqn))* (y(1,j)-
     +      y(1,j-1)*cos(hy*vpoty(1,j-1))-x(1,j-1)*sin(hy*vpoty(1,j-1)))
            sfac = sin(two*pi*vornum* (j-one)/real(ny))
            cfac = cos(two*pi*vornum* (j-one)/real(ny))
...
c        Kinetic Energy Part, at origin (only needed in zero field).
     ...
     +   (-cos(hy*vpoty(1,1)))*fkiny2
         gradax(1,1) = gradax(1,1) + fkinx1*
     +   (hx*x(1,1)*sin(hx*vpotx(1,1))+hx*y(1,1)*cos(hx*vpotx(1,1))) +
     +   fkinx2* (hx*y(1,1)*sin(hx*vpotx(1,1))-
     +   hx*x(1,1)*cos(hx*vpotx(1,1))) + ffield/hy
         graday(1,1) = graday(1,1) + fkiny1*
     +   (hy*x(1,1)*sin(hy*vpoty(1,1))+hy*y(1,1)*cos(hy*vpoty(1,1))) +
     +   fkiny2* (hy*y(1,1)*sin(hy*vpoty(1,1))-
     +   hy*x(1,1)*cos(hy*vpoty(1,1))) - ffield/hx
         fkinx1 = (two/ (hx*hx*sqn))* (x(nx+1,1)-
     +   x(nx,1)*cos(hx*vpotx(nx,1))+y(nx,1)*sin(hx*vpotx(nx,1)))
         fkinx2 = (two/ (hx*hx*sqn))* (y(nx+1,1)-
     +   y(nx,1)*cos(hx*vpotx(nx,1))-x(nx,1)*sin(hx*vpotx(nx,1)))
         fkiny1 = (two/ (hy*hy*sqn))* (x(1,ny+1)-
     +   x(1,ny)*cos(hy*vpoty(1,ny))+y(1,ny)*sin(hy*vpoty(1,ny)))
         fkiny2 = (two/ (hy*hy*sqn))* (y(1,ny+1)-
     +   y(1,ny)*cos(hy*vpoty(1,ny))-x(1,ny)*sin(hy*vpoty(1,ny)))
