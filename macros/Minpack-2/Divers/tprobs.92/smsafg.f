      subroutine smsafg(nx,ny,x,f,fgrad,task,bottom,top,left,right)
      character*(*) task
      integer nx,ny
      real f
      real x(nx*ny),fgrad(nx*ny),bottom(nx+2),top(nx+2),left(ny+2),
     +     right(ny+2)
c     **********
c
c     Subroutine smsafg
...
c     Minimal Surface Area problem.  Minimal surface problems arise
c     from the need to find the surface with minimal area that fits
c     prescribed boundary conditions.  In this subroutine, the domain
c     is the unit square.
c
c     The subroutine statement is:
c
c       subroutine smsafg(nx,ny,x,f,fgrad,task,bottom,top,left,right)
c
c     where
c
...
c     **********
      ...
      integer i,j,k
      real hx,hy,vl,vr,vb,vt,v,dvdx,dvdy,area,fu,fl,xline,yline,alphaj,
     +     betai
...
c     triangular elements.
      ...
      do 50 j = 0,ny
         do 40 i = 0,nx
            ...
            if (task.eq.'F' .or. task.eq.'FG') f = f + fl
            if (task.eq.'G' .or. task.eq.'FG') then
...
c     triangular elements.
      ...
      do 70 j = 1,ny + 1
         do 60 i = 1,nx + 1
            ...
            if (task.eq.'F' .or. task.eq.'FG') f = f + fu
            if (task.eq.'G' .or. task.eq.'FG') then
