...
c
c     **********
      real zero,one,two,three,four,six,ten,twelve,sxteen
      parameter (zero=0.0,one=1.0,two=2.0,three=3.0,four=4.0,six=6.0,
     +           twelve=12.0,sxteen=16.0,ten=10.0)

      integer i,n1,n2
      real alphan,alphas,betan,betas,c,ds,dn,ec,em,fac,gamma,h1,h2,hcs,
     +      hcn,hbar,pi,pens,penn,tcs,tcn
...
c     and (ds, d).
     ...
     +   (betan/ten)* (two*x(i+1)**2+six*x(i+1)*x(i)+twelve*x(i)**2)+
     +   two*gamma/h1/h1)*s(i) + h1* (alphan/three+
     +   (betan/ten)* (three*x(i+1)**2+four*x(i+1)*x(i)+three*x(i)**2)-
     +   two*gamma/h1/h1)*s(i+1)
         y(i+1) = y(i+1) + h1* (alphan/three+
     +   (betan/ten)* (three*x(i+1)**2+four*x(i+1)*x(i)+three*x(i)**2)-
     +   two*gamma/h1/h1)*s(i) + h1* (two*alphan/three+
     +   (betan/ten)* (twelve*x(i+1)**2+six*x(i+1)*x(i)+two*x(i)**2)+
     +   two*gamma/h1/h1)*s(i+1)
   20 continue
      do 30 i = n1 + 1,n1 + n2
         y(i) = y(i) + h2* (two*alphas/three+
     +   (betas/ten)* (two*x(i+1)**2+six*x(i+1)*x(i)+twelve*x(i)**2)+
     +   two*gamma/h2/h2)*s(i) + h2* (alphas/three+
     +   (betas/ten)* (three*x(i+1)**2+four*x(i+1)*x(i)+three*x(i)**2)-
     +   two*gamma/h2/h2)*s(i+1)
         y(i+1) = y(i+1) + h2* (alphas/three+
     +   (betas/ten)* (three*x(i+1)**2+four*x(i+1)*x(i)+three*x(i)**2)-
     +   two*gamma/h2/h2)*s(i) + h2* (two*alphas/three+
     +   (betas/ten)* (twelve*x(i+1)**2+six*x(i+1)*x(i)+two*x(i)**2)+
     +   two*gamma/h2/h2)*s(i+1)
   30 continue
      do 40 i = n1 + n2 + 1,n - 1
         y(i) = y(i) + h1* (two*alphan/three+
