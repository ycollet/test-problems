      subroutine diaofj(m,n,x,fvec,fjac,ldfjac,task,nint)
      character*(*) task
...
c
c       subroutine diaofj(m,n,x,fvec,fjac,ldfjac,task,nint,sigma)
c
c     where
c
...
c     Set up the continuity equations.
  ...
  280    continue
         ...
         do 310 e = 1, s
            do 300 mm = 1, deg(e)
               if (task .eq. 'F' .or. task .eq. 'FJ') 
     +            fvec(eqn(e)+mm) = x(var(e)+npi+mm) - w(mm,e)
               if (task .eq. 'J' .or. task .eq. 'FJ') then
