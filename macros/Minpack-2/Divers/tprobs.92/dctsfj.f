...
c     **********
      ...
      double precision scale1,scale2,zero,one,hund 
      parameter(scale1=4.08d0,scale2=0.417d0,zero=0.0d0,one=1.0d0,
     +          hund=1.0d2)
...
c     Initialization on the first call to the subroutine.
      ...
      if (first) then
         first = .false.
         open(nread,file='dcts.dat',status='old')
         do 10 i = 1, mdiv4
            read(nread,*)indvar(i,1),indvar(i,2),y(i),y(i+mdiv4)
   10    continue
         close(nread)
         do 20 i = 1, mdiv4
